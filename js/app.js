...
const renderRating = (rate) =>{
  // const span = document.getElementById('rateContainer');
  // debugger;
  let spanString = "";
  for(let i = 0; i<parseInt(rate); i++){
    spanString+=`<i id="star" class="fas fa-star"></i>`;
  }
  if(rate > parseInt(rate)){
    spanString+=`<i id="star" class="fas fa-star-half"></i>`
  }
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product mt-4 mx-3">
      <div>
    <img class="product-image" src=${product.image}></img>
      ...
      <h4>Average rate:  ${product.rating.rate}<br><span id="rateContainer"></span>${renderRating(product.rating.rate)}</h4>
      <h5>${product.rating.count} person rating this product</h5>
